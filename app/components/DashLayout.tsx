  ...
  return (
    <>
      <style jsx global>{`
        ...
        .dash-topbar { height: 52px; border-bottom: 1px solid rgba(255, 255, 255, 0.04); display: flex; align-items: center; padding: 0 40px; background: rgba(8, 9, 13, 0.5); backdrop-filter: blur(20px); flex-shrink: 0; }
        ...
        @media (max-width: 900px) { .dash-side { display: none; } .dash-content { padding: 24px 20px; } }
      `}</style>
      ...
      <div className="dash">
        ...
        <aside className="dash-side">
          ...
          <nav className="dash-nav">
            {NAV.map((item) => (
              <button key={item.path} onClick={() => router.push(item.path)} className={`dash-navbtn ${pathname === item.path ? "dash-navbtn-active" : ""}`}>
