...
export default function TrendGraph() {
  ...
  const menuItems = [
    ...
    { icon: "📈", label: "Trends", path: "/trends", active: true },
    ...
    return (
      <svg width="100%" viewBox={`0 0 ${width} ${height}`} className="overflow-visible">
        ...
        {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => (
          <g key={i}>
            <line x1={padding.left} y1={padding.top + graphH * (1 - pct)} x2={padding.left + graphW} y2={padding.top + graphH * (1 - pct)} stroke="rgba(255,255,255,0.05)" strokeWidth="1" />
            <text x={padding.left - 8} y={padding.top + graphH * (1 - pct) + 4} textAnchor="end" fontSize="10" fill="rgba(255,255,255,0.3)">{Math.round(max * pct / 1000)}K</text>
          </g>
        ))}
        ...
        {points.map((p: any, i: number) => (
          <g key={i}>
            ...
            {i % 3 === 0 && <text x={p.x} y={height - 8} textAnchor="middle" fontSize="9" fill="rgba(255,255,255,0.3)">{p.month.slice(0, 7)}</text>}
          </g>
        ))}
      </svg>
    );
        ...
        {result && !loading && (
          <div className="space-y-6">
            <div className={`border rounded-2xl p-5 flex items-center gap-4 ${verdictConfig[result.verdict].bg}`}>
              <div className={`text-4xl font-black ${verdictConfig[result.verdict].color}`}>{verdictConfig[result.verdict].icon}</div>
              <div>
